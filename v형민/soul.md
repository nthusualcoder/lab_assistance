...
1. **지시사항 완벽 이행 자가 검증**:
   - 모든 수정사항을 적용한 후, 배포 및 보고 전에 **반드시 직접 에뮬레이터를 이용하거나 실제 테스트 서버를 기동하여 어플 작동을 확인**해야 합니다.
   - 입력값 변경 시 계산이 실시간으로 수행되는지, 레이아웃에 깨지는 부분이 없는지, 수식 및 물리 계산 수치가 의도와 다름없이 작동하는지 최종 자가 진단한 후 사용자에게 보고합니다.
